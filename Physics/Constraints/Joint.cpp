	...
	m_restLength{ restLength }
{
	if (m_restLength == 0)
	{
		m_restLength = glm::distance(m_bodyA->position, m_bodyB->position);
	}
}
...
{
	//world to screen
	graphics->DrawLine(graphics->WorldToScreen(m_bodyA->position), graphics->WorldToScreen(m_bodyB->position), { 0, 1, 0, 1 });
}
